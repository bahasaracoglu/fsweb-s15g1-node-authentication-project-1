...
async function idyeGoreBul(user_id) {
  let foundUser = await db("users").where("user_id", user_id).first();
  return foundUser;
}
...
async function ekle(user) {
  let [user_id] = await db("users").insert(user); //burada insert metodu id'yi içeren bir array döner. let [user_id] ile direkt içindeki değer alabilirim.
